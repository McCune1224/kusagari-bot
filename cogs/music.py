...
class Player(commands.Cog):
    ...
    async def check_queue(self, ctx):
        """Function for getting the length of current song queue"""

        if len(self.song_queue[ctx.guild.id]) > 0:
            ctx.voice_client.stop()
            await self._play_song(ctx, self.song_queue[ctx.guild.id][0])
            self.song_queue[ctx.guild.id].pop(0)

    async def _search_song(self, amount, song, get_url=False):
        """Helper function for search() to grab the info via lambda function and youtube_dl library to pull info of a youtube search based off string of song name given"""
        ...
        # enteries found, return a list of said entries
        return [entry["webpage_url"] for entry in info["entries"]] if get_url else info

    async def _play_song(self, ctx, song):
        """Internal function to play song via discord.PCMVolumeTransofmrer and discord.FFmpegPCMAudio (requires ffmpeg to be installed on host's device)"""
    ...
    async def join(self, ctx):
        """Attempt join to requested user's current voice channel."""
    ...
    async def leave(self, ctx):
        """Disconnect Client from requested channel"""
    ...
    async def play(self, ctx, *, song=None):
        ...
        if not ("youtube.com/watch?" in song or "https://youtu.be/" in song):
            ...
            result = await self._search_song(1, song, get_url=True)
    ...
    async def search(self, ctx, *, song=None):
        """Will return a list of links to use for playing a song"""
        ...
        info = await self._search_song(5, song)
    ...
    async def queue(self, ctx):
        """Display the current song queue for Guild"""
    ...
    async def skip(self, ctx):
        """Voting system to skip the current song of queue"""
